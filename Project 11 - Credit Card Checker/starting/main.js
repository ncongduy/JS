...
const validateCred = (check) => {
  let sum = 0;
  for (let i = check.length - 1; i >= 0; i = i - 2) {
    if (i === 0) {
      sum += check[0];
      break;
    }
    let right = check[i];
    let left = check[i - 1] * 2;
    if (left > 9) {
      left -= 9;
    }
    sum += right + left;
...
const findInvalidCards = (findInvalid) => {
  let cardInvalid = [];
  for (let i = 0; i < findInvalid.length; i++) {
    if (!validateCred(findInvalid[i])) {
      cardInvalid.push(findInvalid[i]);
    }
  }
  return cardInvalid;
};

const idInvalidCardCompanies = (cardNumber) => {
  let cardInv = findInvalidCards(cardNumber);
  let company = [];
